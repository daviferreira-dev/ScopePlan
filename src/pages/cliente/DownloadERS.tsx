 ...
 *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
 :root {
 --green-deep: #1a5c2a; --green-mid: #2d7a40; --green-bright: #3a9e52; --green-light: #4ebb68;
 --sidebar-bg: #1e6030; --card-bg: #ffffff; --card-border: #e8f0ea;
 --text-primary: #1a2e1f; --text-muted: #7a9982; --text-sidebar: rgba(255,255,255,0.75);
 --progress-bg: #e4f0e7;
 }
...
export default function DownloadERS({ project, requirements, onBack }: Props) {
  ...
  const topicsWithCount: TopicInfo[] = BASE_TOPICS.map((t) => ({
    ...t,
    count: requirements.filter((r) => r.status === "aprovado" && r.tipo === t.type).length,
  }));
  ...
  const handleDownload = async () => {
    ...
    try {
      // Map selected topic IDs to requirement IDs (approved only)
      const topicIdToType: Record<number, string> = {
        1: "funcional",
        2: "negocio",
        3: "nao_funcional",
        4: "restricao",
      };
      const selectedTypes = selectedIds.map((id) => topicIdToType[id]);
      const requirementIds = requirements
        .filter((r) => r.status === "aprovado" && selectedTypes.includes(r.tipo))
        .map((r) => r.id);

      const blob = await projectsApi.downloadERS(project.id, format, requirementIds);
  ...
  const getUserInitials = () => {
    if (!user?.nome) return "CL";
    return user.nome.split(" ").map((n) => n[0]).join("").slice(0, 2).toUpperCase();
  };

  const getUserRole = () => {
    if (user?.perfil === "cliente") return "Cliente";
    return user?.perfil || "";
  };
        ...
        <aside className={`sidebar ${sidebarOpen ? "open" : ""}`}>
          ...
          <div className="sidebar-user">
            ...
            <div className="user-info">
              <div className="user-name">{user?.nome || "Cliente"}</div>
              ...
              <div className="download-right">
                <div className="preview-panel">
                  ...
                  {selectedTopics.map((topic) => (
                    <div key={topic.id} className="abnt-section">
                      <h2>{topic.name}</h2>
                      <ul className="abnt-file-list">
                        {requirements
                          .filter((r) => r.tipo === topic.type && r.status === "aprovado")
                          .map((req) => (
                            <li key={req.id} className="abnt-file-item">
                              <span className="abnt-file-bullet">•</span>
                              <span className="abnt-file-name">{req.codigo} - {req.titulo}</span>
                            </li>
                          ))}
                        {requirements.filter((r) => r.tipo === topic.type && r.status === "aprovado").length === 0 && (
                          <li className="abnt-no-files">Nenhum requisito aprovado</li>
                        )}
                      </ul>
                    </div>
                  ))}
